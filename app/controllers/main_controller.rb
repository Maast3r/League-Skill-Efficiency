...
class MainController < ApplicationController
	...
	def getVersion
		...
		if response.code == 200
			...
			versionExists = Version.find_by(version: response[0])
			if versionExists
				versionExists.update(version: response[0])
			else
  ...
  def getChamps
		...
		if response.code == 200
			parse_API_response(response.parsed_response)
		end
  end
			...
			champion["spells"].each do |spells|
				...
				spells["leveltip"]["effect"].each_with_index do |effect, index|
					if (effect.include? "e") &&
						...
						if spells["leveltip"]["label"][index].to_s.include? "Bonus Damage"
							bonus = true
						end
						if dmgCounter == 0
				...
				spells["cooldown"].each_with_index do |cd, index|
					...
					if spells["cooldown"].ensure_array[5] == nil
						cooldown[5] = cooldown[4]
					end
				end
	...
	def getDmg(dmg, effect, spells, divideBy)
		...
		dmg[0] = spells["effect"].ensure_array[eIndex].ensure_array[0] != nil ? spells["effect"].ensure_array[eIndex].ensure_array[0]/divideBy : 0;
		dmg[1] = spells["effect"].ensure_array[eIndex].ensure_array[1] != nil ? spells["effect"].ensure_array[eIndex].ensure_array[1]/divideBy : 0;
		dmg[2] = spells["effect"].ensure_array[eIndex].ensure_array[2] != nil ? spells["effect"].ensure_array[eIndex].ensure_array[2]/divideBy : 0;
		if spells["effect"].ensure_array[eIndex].ensure_array[3] == nil
			dmg[3] = dmg[2]
		else
			dmg[3] = spells["effect"].ensure_array[eIndex].ensure_array[3]
		end
		if spells["effect"].ensure_array[eIndex].ensure_array[4] == nil
			dmg[4] = dmg[3]
		else
			dmg[4] = spells["effect"].ensure_array[eIndex].ensure_array[4]
		end
		if spells["effect"].ensure_array[eIndex].ensure_array[5] == nil
			dmg[5] = dmg[4]
		else
